...
class WHUOptSarPatchDataset(Dataset):
    ...
    def __init__(
        ...
        self.output_size = output_size if output_size is not None else patch_size
        if self.output_size <= 0:
            raise ValueError("output_size must be a positive integer.")
        if self.output_size > self.patch_size:
            raise ValueError("output_size cannot be larger than patch_size.")
    ...
    def _generate_sar_transform(self) -> np.ndarray:
        """Generate the same type of random affine + perspective transform used above."""
        ...
        tx = random.randint(-displacement_range, displacement_range)
        ty = random.randint(-displacement_range, displacement_range)
        rotation_matrix[0, 2] += tx
        rotation_matrix[1, 2] += ty

        affine_src_points = cv2.transform(src_points.reshape(1, -1, 2), rotation_matrix).reshape(-1, 2)
    ...
    def _apply_sar_transform(self, sar_patch: np.ndarray) -> np.ndarray:
        """Apply the random transform to SAR patch while preserving channel layout."""
        matrix = self._generate_sar_transform()
        ...
        if sar_patch.ndim == 3:
            ...
            sar_hwc = np.transpose(sar_patch, (1, 2, 0))
            warped = cv2.warpPerspective(
                sar_hwc,
                matrix,
                (self.patch_size, self.patch_size),
        ...
        raise ValueError(f"SAR patch must be (C, H, W) or (H, W), got {sar_patch.shape}.")

    def _center_crop(self, arr: np.ndarray, crop_size: int) -> np.ndarray:
        """Center crop a (C,H,W) or (H,W) array to the requested size."""
        ...
        if arr.ndim == 3:
            ...
            raise ValueError(f"Unsupported array shape for cropping: {arr.shape}")

        if crop_size > h or crop_size > w:
            raise ValueError(f"crop_size {crop_size} cannot be larger than input shape {(h, w)}.")
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.add_dist:
            sar_patch = self._apply_sar_transform(sar_patch)

        optical_patch = self._center_crop(optical_patch, osz)
        sar_patch = self._center_crop(sar_patch, osz)
        label_patch = self._center_crop(label_patch, osz)
...
class Houston2013PatchDataset(Dataset):
    ...
    def _generate_lidar_transform(self) -> np.ndarray:
        """Generate a random perspective transform for the LiDAR patch."""
        size = self.patch_size
        src_points = np.float32([
            [20, 20],
            [size - 20, 20],
    ...
    def _apply_lidar_transform(self, lidar_patch: np.ndarray) -> np.ndarray:
        """Apply the random geometric transform to the LiDAR patch."""
        matrix = self._generate_lidar_transform()

        if lidar_patch.ndim == 3 and lidar_patch.shape[0] == 1:
            lidar_2d = lidar_patch[0]
            warped = cv2.warpPerspective(
                lidar_2d,
                matrix,
                (self.patch_size, self.patch_size),
        ...
        raise ValueError(f"LiDAR patch must be (1, H, W) or (H, W), got {lidar_patch.shape}.")
    ...
    def __getitem__(self, idx: int):
        ...
        if self.add_dist:
            lidar_patch = self._apply_lidar_transform(lidar_patch)

        hsi_patch = self._center_crop(hsi_patch, osz)
        lidar_patch = self._center_crop(lidar_patch, osz)
        label_np = self._center_crop(label_np, osz)
